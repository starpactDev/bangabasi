								...
								<tbody>
									@foreach($usersWithSalesAndPrice as $user)
										<tr>
											...
											<td class="text-dark d-none d-md-block">{{'₹'.number_format($user->total_price)}}</td>
										</tr>
									@endforeach
								...
								<div class="row media d-flex pt-15px pb-15px">
									...
									<div class="col-lg-9 col-md-9 col-10 media-body align-self-center ec-pos">
										...
										<p class="mb-0 ec-price">
											<span class="text-dark">₹{{ number_format($products->product->offer_price) }}</span>
											<del>₹{{ number_format($products->product->original_price) }}</del>
										</p>
									</div>
								</div>
                            @endforeach
			...
			<div class="row py-4">
				<div class="col-xl-6">
					...
					<div class="card ec-cust-card card-table-border-none card-default">
						<div class="card-header justify-content-between ">
							<h2>New Sellers</h2>
							<h2>{{number_format($sellerCount) ?? 0}} Listed</h2>
						</div>
						<div class="card-body pt-0 pb-15px">
							<table class="table ">
								<thead>
								...
								<tbody>
									@foreach($newSellers as $user)
										<tr>
											...
											<td class="text-dark d-none d-md-block">{{number_format($user->total_products) ?? 0}}</td>
											<td>{{ number_format($user->total_sales) ?? 0}}</td>
											<td>{{ '₹'.number_format($user->total_sale_price) ?? 0 }}</td>
										</tr>
									@endforeach
