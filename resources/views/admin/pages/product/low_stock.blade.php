		...
		<div class="breadcrumb-wrapper d-flex align-items-center justify-content-between">
			<div>
				...
				<p class="breadcrumbs"><span><a href="{{ route('admin_dashboard') }}">Home</a></span>
					<span><i class="mdi mdi-chevron-right"></i></span>Product Stock
				</p>
			</div>
		...
		<div class="row">
			<div class="col-12">
				<div class="card card-default">
					...
					<div class="card-body">
						<div class="alert alert-danger" role="alert" style="background-color: #dc3545; color: white;">
							<strong>Warning!</strong> The following products are running low in stock (less than 5 available):
						</div>
							...
							<table id="responsive-data-table" class="table" style="width:100%">
								...
								<tbody>
									...
									<tr>
										...
										<td>{{ $product->name ?? '' }}</td>
										<td>{{ $product->categoryDetails->name ?? ''}}
											<p>{{ $product->subCategoryDetails->name ?? '' }}</p>
										</td>
										...
										<td>
											...
											@if ($size->quantity < 5)
												<span style="color: rgb(21, 21, 105); font-weight:600; margin-bottom:8px">
												{{ $size->size }}
												</span>
